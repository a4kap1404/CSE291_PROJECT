    ...
    for j in $(seq 1 5); do
        formatted_counter_j=$(printf "%.2f" "$counter_j")
            for k in $(seq 1 4); do
                ...
                (
                    echo "Deleting $FLOW_VARIANT with utilization $formatted_counter_j, aspect ratio $formatted_counter_i and place density $formatted_counter_k"
                    export FLOW_VARIANT=$FLOW_VARIANT
                    DESIGN_CONFIG="$new_config" make clean_all
